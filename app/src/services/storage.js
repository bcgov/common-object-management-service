 ...
 */
  ...
  /**
   * @function removePublicAcl
   * remove existing public-read acl on object if present (phase out previous implementation)
   * @param {object} data An object containing accessKeyId, bucket, endpoint, key,
   * @param {string} key Key of the resource
   * @returns {Promise<boolean>} whether a publci ACL was removed
  ...
  async updatePublic({ path, public: publicFlag = false, bucketId }) {
    ...
    // Update policy
    // filter out any existing rules for this resource or below
    ...
    if (!isPrefix) await this.removePublicAcl(data, path);
  ...
  /**
   * @function getPublic
   * checks for a Bucket Policy that will make the given resource public
   * @param {string} path the path of the resource
   * @param {string} bucketId of COMS bucket for the resource
