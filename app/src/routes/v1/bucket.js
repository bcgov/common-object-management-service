...
/** Creates a bucket */
router.put('/', checkS3BasicAccess, express.json(), bucketValidator.createBucket, (req, res, next) => {
 ...
 */
router.head('/:bucketId', bucketValidator.headBucket, checkS3BasicAccess,
  hasPermission(Permissions.READ), (req, res, next) => {
    bucketController.headBucket(req, res, next);
  });

/** Returns a bucket */
router.get('/:bucketId', bucketValidator.readBucket, checkS3BasicAccess,
  hasPermission(Permissions.READ), (req, res, next) => {
    bucketController.readBucket(req, res, next);
  });

/** Search for buckets */
router.get('/', bucketValidator.searchBuckets, checkS3BasicAccess, (req, res, next) => {
  bucketController.searchBuckets(req, res, next);
});

/** Updates a bucket */
router.patch('/:bucketId', express.json(),
  bucketValidator.updateBucket, checkS3BasicAccess, hasPermission(Permissions.MANAGE),
  (req, res, next) => {
...
/** Deletes the bucket */
router.delete('/:bucketId', checkS3BasicAccess, bucketValidator.deleteBucket,
  hasPermission(Permissions.DELETE), (req, res, next) => {
    bucketController.deleteBucket(req, res, next);
  });

/** Creates a child bucket */
router.put('/:bucketId/child', express.json(), bucketValidator.createBucketChild,
  checkS3BasicAccess, hasPermission(Permissions.MANAGE),
  (req, res, next) => {
...
router.get('/:bucketId/sync', bucketValidator.syncBucket,
