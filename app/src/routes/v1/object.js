...
/** Creates new objects */
router.put('/', requireSomeAuth, checkS3BasicAccess, objectValidator.createObject,
  currentUpload(true), (req, res, next) => {
    objectController.createObject(req, res, next);
  });

/** Search for objects */
router.get('/', requireSomeAuth, objectValidator.searchObjects, checkS3BasicAccess, (req, res, next) => {
  objectController.searchObjects(req, res, next);
});

/** Fetch metadata for specific objects */
router.get('/metadata', requireSomeAuth, objectValidator.fetchMetadata, checkS3BasicAccess, (req, res, next) => {
  objectController.fetchMetadata(req, res, next);
});

/** Fetch tags for specific objects */
router.get('/tagging', requireSomeAuth, objectValidator.fetchTags, checkS3BasicAccess, (req, res, next) => {
  objectController.fetchTags(req, res, next);
});

/** Returns object headers */
router.head('/:objectId', objectValidator.headObject, currentObject,
  checkS3BasicAccess, hasPermission(Permissions.READ),
  (req, res, next) => {
...
router.get('/:objectId', helmet({ crossOriginResourcePolicy: { policy: 'cross-origin' } }),
  objectValidator.readObject, currentObject, checkS3BasicAccess, hasPermission(Permissions.READ),
  (req, res, next) => {
...
/** Updates an object */
router.put('/:objectId', requireSomeAuth, objectValidator.updateObject, currentUpload(),
  currentObject, checkS3BasicAccess, hasPermission(Permissions.UPDATE), (req, res, next) => {
...
router.delete('/:objectId', requireSomeAuth, objectValidator.deleteObject,
  currentObject, checkS3BasicAccess, hasPermission(Permissions.DELETE), (req, res, next) => {
...
router.get('/:objectId/version', requireSomeAuth, objectValidator.listObjectVersion,
  currentObject, checkS3BasicAccess, hasPermission(Permissions.READ), (req, res, next) => {
...
/** Add tags to an object */
router.delete('/:objectId/tagging', requireSomeAuth, objectValidator.deleteTags,
  currentObject, checkS3BasicAccess, hasPermission(Permissions.UPDATE), (req, res, next) => {
