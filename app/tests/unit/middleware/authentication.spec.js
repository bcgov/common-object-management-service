...
describe('currentUser', () => {
  ...
  describe('OIDC Authorization', () => {
    ...
    it.each([
      ...
      expect(jwtVerifySpy).toHaveBeenNthCalledWith(1, expect.any(String), publicKey, expect.objectContaining({
        issuer: `${serverUrl}/realms/${realm}`
      }));
      expect(jwtVerifySpy).toHaveBeenNthCalledWith(2, expect.any(String), undefined, expect.objectContaining({
        algorithms: ['none']
      }));
    ...
    it('sets authType to BEARER without keycloak.publicKey and valid token', async () => {
      ...
      expect(jwtVerifySpy).toHaveBeenCalledWith(expect.any(String), undefined, expect.objectContaining({
        algorithms: ['none']
      }));
      expect(loginSpy).toHaveBeenCalledTimes(1);
      expect(loginSpy).toHaveBeenCalledWith(expect.objectContaining({ sub: 'sub' }));
